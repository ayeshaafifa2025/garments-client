...
const ManageUsers = () => {
    ...
    const handleStatusUpdate = async (id, updateFields, successMsg) => {
        try {
            ...
            Swal.fire({
                icon: "error",
                title: "Failed!",
                text: "Could not update user status/role.",
            });
        }
    };
    ...
    const handleSuspend = async (user) => {
        const { value: formValues } = await Swal.fire({
            ...
            html:
                '<input id="swal-input1" class="swal2-input" placeholder="Suspend Reason (e.g. Policy Violation)">' +
                '<textarea id="swal-input2" class="swal2-textarea" placeholder="Suspending Admin Feedback"></textarea>',
            focusConfirm: false,
        ...
        if (formValues) {
            const updateFields = {
                status: 'suspended',
                suspendReason: formValues.suspendReason,
                suspendFeedback: formValues.suspendFeedback
            };
            handleStatusUpdate(user._id, updateFields, `${user.displayName} has been suspended.`);
        }
    }
    
    const handleApprove = (user) => {
        const newRole = user.demandedRole;
        const updateFields = {
            role: newRole,
            status: 'active',
            demandedRole: null
        };
        handleStatusUpdate(user._id, updateFields, `${user.displayName} is now an ${newRole}.`);
    }
    ...
    //         status: 'active'
    //     };
    //     handleStatusUpdate(user._id, updateFields, `${user.displayName} is now an admin.`);
    // }
      ...
      <div>
        <Helmet>
          ...
          <div className="p-4 md:p-8">
                ...
                <table className="table w-full">
                    <thead>
                        <tr className="bg-gray-200 text-gray-700 uppercase text-sm leading-normal">
                            ...
                            <th className="py-3 px-6 text-center">Current Role</th>
                            <th className="py-3 px-6 text-center">Demanded Role</th> 
            ...
            "
                                                data-tip={`Approve as ${user.demandedRole}`}
                                            >
                                        ...
                                        {isSuspended(user) && (
                                            <button 
                                                onClick={() => handleStatusUpdate(
                                                    user._id, 
                                                    { status: 'active', suspendReason: null, suspendFeedback: null }, 
            ...
            "
                ...
                {users.length === 0 && (
                    <p className="text-center py-8 text-lg text-gray-500">No users found matching the filters.</p>
                )}
            </div>
        </div>
      </div>
